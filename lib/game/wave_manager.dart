...
class WaveManager extends Component with HasGameRef<MegamaniaGame> {
  ...
  void update(double dt) {
    ...
    if (!hasActiveEnemies) {
      ...
      double minEnemyY = double.infinity;
      double maxEnemyY = 0.0;
      for (final enemy in enemies) {
        if (enemy.position.y < minEnemyY) {
          minEnemyY = enemy.position.y;
        }
  ...
  void _spawnAllEnemies() {
    ...
    if (phaseType == 0) {
      ...
      // Other phases: Grid of 2 rows of enemies
      int cols = (availableWidth / 55).floor() + 1;
      cols = cols.clamp(6, 12);
      const int rows = 2;
      final double spacingX = availableWidth / (cols - 1);
      const double spacingY = 45.0; // Vertical spacing between rows
